...
open class ExpandingMenuButton: UIView, UIGestureRecognizerDelegate {
    ...
    open var bottomViewAlpha: CGFloat = 1.0
    ...
    public init(frame: CGRect, centerImage: UIImage, centerHighlightedImage: UIImage) {
        ...
        func configureViewsLayoutWithButtonSize(_ centerButtonSize: CGSize) {
            ...
            self.backgroundImageView = UIImageView(frame: self.bottomView.frame)
            
            self.blurView = FXBlurView(frame: self.bottomView.frame)
            self.blurView.blurRadius = 10
            self.blurView.tintColor = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0)
            self.blurView.isDynamic = false
            
            self.masksView = UIView(frame: self.bottomView.frame)
            self.masksView.backgroundColor = UIColor(red: 29/255, green: 29/255, blue: 38/255, alpha: 0.65)
            
            self.bottomView.addSubview(self.backgroundImageView)
            self.bottomView.addSubview(self.blurView)
            self.bottomView.addSubview(self.masksView)
    ...
    fileprivate func expandMenuItems() {
        self.backgroundImageView.image = ImageHelper.captureView()
